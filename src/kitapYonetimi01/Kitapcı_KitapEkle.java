...
public class Kitapcı_KitapEkle {
	...
	public static List<EklenenKitaplar> kitapEkle() {
		...
		do {
			...
			System.out.println("Sırayla Lütfen Kitap adını, yazarını ve fiyatını giriniz\n"
	...
	public static void bilgiIleKitap() {
		...
		Scanner scan = new Scanner(System.in);
		do {
		String yazar1 = " ";//Declaration, assignment
		String kitap = " ";
		int fiyat = 0;//initialize etmek, ilk değerini verdik
		boolean flag = true;
		while(flag==true){
			...
			case "3":
				...
				flag=false;
				break;
			default: //yanlış işlem ihtimali
				System.out.println("Geçerli bir işlem giriniz:");		
			}
		}
			
			boolean gecerliMi=false;
			for (EklenenKitaplar kitap1 : liste) {//ignore=önemsememek
	...
	public static void numaraIleKitapSil() {
		...
		while (sil == false) {
			...
			for (int i = 0; i < liste.size(); i++) { //Remove: Kaldırmak, silmek
	...
	public static void tumKitaplariListele() {
		...
		for (EklenenKitaplar kitap1 : liste) {//for each döngüsü
